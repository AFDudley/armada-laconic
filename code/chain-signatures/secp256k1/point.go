...
// ordinate and a byte which is 2 if Y is even, 3 if it's odd. Or it returns an
// error on failure.
func (P *secp256k1Point) MarshalBinary() ([]byte, error) {
	...
	signByte := 0 // First byte contains sign of Y.
	xordinate := P.X.Bytes()
	copyLen := copy(rv[1:], xordinate[:])
...
func (P *secp256k1Point) UnmarshalBinary(buf []byte) error {
	...
	signByte := 0 // First byte contains sign of Y.
	if err == nil && !(buf[signByte] == 2 || buf[signByte] == 3) {
		err = fmt.Errorf("bad sign byte")
	}
	if err != nil {
		return err
	}
	...
	P.Y.Set(maybeY)
	if (buf[signByte] == 2 && !isEven) || (buf[signByte] == 3 && isEven) {
		P.Y.Neg(P.Y)
	} else if buf[signByte] != 2 && buf[signByte] != 3 {
		return fmt.Errorf("parity byte must be 2 or 3")
	}
	return nil
}
