...
| WP | §5 items | Note |
|---|---|---|
| **A · nitro-on-railgun** | T0.0–T0.3, T0.6 *(opt)*, T0.7; T6.2, T6.3 | Our own pool + circuits + own Phase-2 (ADR-0014); the deposit/payout boundary is the net-new integration contract (ADR-0002/0003/0004). |
...
| **C · yield & exchange** | T4.0–T4.6, T3.\* *(v2)*; **T5.0 adapters (CCTP built · Aave-v4 yield · Swaps), T5.1 routing** | Posted-price venue v1; adapters build or reuse Railgun's Cookbook recipe. **USDC yield is the priority** (ADR-0007/0011/0013). |
...
| Component | Note |
|---|---|
| **Railgun design** (`contract` / `circuits-v2` spec; `engine` / `cookbook` as reference) | **Reference spec only** — the shielded pool + JoinSplit circuits are our own implementation (T0.0/T0.1, spec-compatible with the Railgun design), not reused OSS (ADR-0014). |
...
| Railgun **RelayAdapt / Cookbook** recipe | Railgun's own value-moving recipe; the C adapters may reuse it, distinct from our T0.3 deposit/payout contract (ADR-0010). |
...
|---|---|---|---|
| **Ethereum L1** | Settlement root: our own pool, `go-nitro` adjudicator, and deposit/payout contract live here. Bidirectional — write channel state, read commitment/nullifier/adjudicator events. | T0.0, T0.2, T0.3 (read at T1.2) | ADR-0002/0004/0014 |
