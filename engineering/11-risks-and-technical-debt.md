...
| R9 | **Own pool + circuits (net-new crypto-engineering)** — T0.0/T0.1 are our own implementation of the Railgun design (ADR-0014): independently authored, spec-compatible shielded-pool contracts + JoinSplit circuits + our own Phase-2. This is real, audit-critical crypto-engineering — a subtle spec deviation (note/commitment format, nullifier semantics, `snarkSafetyVector`, verifier wiring) is a soundness or fund-safety bug. | High (a soundness/format bug = unprovable spend or value creation) | Medium — a well-understood, de-risked design (A.1 pins exact behavior/format), but net-new code | Treat A.1 as the reference spec and behavioral oracle; author our own conformance suite (spec-compatibility + fee=0/POI); freeze the circuit subset early; **external crypto audit gates mainnet**. | T0.0 / T0.1 · ADR-0014 |

## Technical debt / known shortcuts

Each of the following is accepted for v1: a scoped, reversible interim
