...
| R2 | **go-nitro maturity gaps** — a **multi-asset ForceMove *app*** is net-new: the holder/outcome machinery (`MultiAssetHolder` / `transferAllAssets` / `protocols/swap/swap.go`) **already supports** multi-asset ETH-in/USDC-out, but no shipped *app* does (`HashLockedSwap` is single-asset/2-party); **dispute wiring** exists in the adjudicator but is not driven end-to-end in the node; and **watchtower response** plus **persistent connections/liveness** are unproven for our flow. go-nitro is documented pre-production. | High (settlement is spend-authorizing; a gap = stuck or lost funds) | Medium — primitives exist, our wiring/app does not | Treat all four as **pre-mainnet blockers, not spine-demo blockers**; author the multi-asset settlement *app* first (the swap-demo pull) on top of the existing holder; drive challenge/response E2E on fixturenet; watchtower response is T6.3 gated on T2 freshness; liveness ties to transport. | T0.2 / T0.3 / T6.3 · ADR-0004 |
...
| R5 | **Audit surface** — anything **spend-authorizing** is audit-critical before mainnet: the T0.3 deposit/payout contract (the notes-in/notes-out boundary), the clean-room T0.0 pool + T0.1 circuits (see R9), and the T6.1 WebView↔RN key bridge / T6.0 enclave custody. | Critical (a leak here = drained funds or leaked keys) | Low if audited, catastrophic if skipped | Freeze spend-authorizing surface early; audit the clean-room pool/circuits for spec-compatibility and soundness (R9); narrow, audited bridge with raw key material never crossing into the WebView; **external audit gates mainnet**, not the demo. | T0.0 / T0.1 / T0.3 / T0.6 / T6.0 / T6.1 · ADR-0014 |
...
| R9 | **Clean-room pool + circuits (net-new crypto-engineering)** — Railgun's `contract` / `circuits-v2` are unlicensed (A.1.10), so T0.0/T0.1 are **clean-room reimplemented** (ADR-0014): independently authored, spec-compatible shielded-pool contracts + JoinSplit circuits + our own Phase-2. This is real, audit-critical crypto-engineering — a subtle spec deviation (note/commitment format, nullifier semantics, `snarkSafetyVector`, verifier wiring) is a soundness or fund-safety bug. | High (a soundness/format bug = unprovable spend or value creation) | Medium — a well-understood, de-risked design (A.1 pins exact behavior/format), but net-new code | Treat A.1 as the reference spec and behavioral oracle; author our own conformance suite (spec-compatibility + fee=0/POI); freeze the circuit subset early; **external crypto audit gates mainnet**. Licensing itself is **resolved** by going clean-room — no longer an open gate. | T0.0 / T0.1 · ADR-0014 |
