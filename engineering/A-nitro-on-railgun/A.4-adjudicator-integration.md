...
Two deltas, both flagged in [A.1.9](./A.1-reuse-inventory.md#a19-net-new-deltas-what-a-actually-builds) §2 and A.0.5 gate 3. Everything else in A.4.3 is reuse.

1. **Multi-asset ForceMove settlement app (ETH-in / USDC-out).** The escrow (`MultiAssetHolder`), the payout loop (`transferAllAssets`), and the off-chain negotiation (`swap.go`) all already support multiple assets. The A.1.8 §2 correction applies here: the holder is multi-asset already; the missing piece is the app. No shipped ForceMove app produces a multi-asset atomic outcome; `HashLockedSwap.sol` (`examples/…` L33–83) is single-asset and 2-party. T0.2 authors an `IForceMoveApp` whose `stateIsSupported` validates a state that debits ETH and credits USDC in one outcome, so the exchange leg (C, T4.1) can settle atomically over reused clearing. C authors the real quote/settle variant against this same `IForceMoveApp` interface and registry; T0.2 ships the reference app and the registration mechanism.
...
- **Multi-asset app is net-new.** The holder and protocol are multi-asset; the app is the missing piece (delta 1). The app is the net-new part; go-nitro outcomes themselves are already multi-asset (A.1.8 §2 correction).
...
**Explicitly reported non-claims:** T0.2 does not demonstrate an automatic watchtower response or full-challenge-window liveness; those are T6.3 and A.6. The joined deposit-from-shielded-note skeleton depends on our own pool and circuits (A.4.2, A.1.10) for the pool side. The adjudicator lifecycles above run independently of that dependency and are the deliverable proof for T0.2.

## A.4.7 Risks / open

- **Build dependency on the pool and circuits.** The adjudicator is independently deployable; the coupled note-to-deposit path waits on our own pool and circuit builds (A.2/A.3). Track the build dependency, but do not block T0.2's own acceptance.
