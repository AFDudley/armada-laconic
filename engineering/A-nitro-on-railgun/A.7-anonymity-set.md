...
The load-bearing distinction this doc defends end-to-end is that **liquidity import is not anonymity-set inheritance** (A.7.4). Accepting Railgun's value is a public boundary hop, never native ingestion of their crowd.
...
3. **Gated-entry enforcement.** A deposit-in carrying a valid POI proceeds to `MultiAssetHolder.deposit`; one lacking a valid POI, or checked against a stale allow-list root, is rejected in the settlement path before escrow (with A.5). This confirms the A.7-to-A.5 ICD is load-bearing.
...
- **Licensing gate (Phase-0), resolved via clean-room ([ADR-0014](../09-architecture-decisions.md#adr-0014)).** Per A.7.2, the pool the crowd lives in is clean-room reimplemented, with no Railgun grant needed. The residual dependency is only that the clean-room pool ships before real-value bootstrap or import.
