...
2. The **trivial single-asset ForceMove app**, a HashLockedSwap-grade stand-in for C's T4.1 quote/settle app used solely by the skeleton. It is cited from [A.1.3](./A.1-reuse-inventory.md) `HashLockedSwap.sol` L33–83 and is not shipped as product (A.1.8 §2: no multi-asset ForceMove app ships).
3. The **fixturenet acceptance harness and adversarial suite** (A.8.6): the standing regression that gates every A merge.
...
**(a) Deposit/payout contract: address and ABI (see A.5).**
The net-new T0.3 contract is A's public-side ERC20 counterparty of unshield/shield and of channel deposit/conclude ([A.1.4](./A.1-reuse-inventory.md)). Consumers get two things: its deployed address and its ABI. The address is the value set as `unshieldPreimage.npk = bytes32(uint160(T0.3))` for deposit-in, and it names the external destination for payout-out. The full ABI is authoritative in A.5. The boundary methods are the fund path and the payout path. The fund path is `IERC20.approve` followed by `MultiAssetHolder.deposit(asset, channelId, expectedHeld, amount)`; the payout path is `shield([ShieldRequest…])` on receipt of the outcome transfer. Here `channelId = NitroUtils.getChannelId(fixedPart)`.

**(b) Boundary events — `Deposit` / `Payout`.**
Emitted by T0.3 so that B (T1.2) can index the crossing between Railgun and Nitro without decoding pool internals:
...
**(c) Channel-lifecycle API (see A.6).**
This is the settlement-client (T6.2) surface that siblings drive to move value through a channel. It reuses ts-nitro `node.ts` fund/defund/pay ([A.1.5](./A.1-reuse-inventory.md)) plus the net-new in-browser dispute surface ([A.1.9](./A.1-reuse-inventory.md) §6):
- **open / fund-from-note** — unshield a note into T0.3 and `deposit` into a freshly-derived `channelId` (directfund/virtualfund shape).
- **propose / accept** — exchange signed `VariablePart`s over the app; the signing domain is `NitroUtils.hashState(fixedPart, variablePart)` (`ForceMove.sol` L236–268), which C's T4.1 app and D's client MUST match.
- **cooperative close** — co-sign a final state; `concludeAndTransferAllAssets` routes the outcome to T0.3 for re-shield, with no on-chain dispute.
- **force-close** — `challenge(FixedPart, proof, candidate, challengerSig)` starting the `finalizesAt` window (A.8.6.3 exit).
- **respond** — `checkpoint(FixedPart, proof, candidate)` overwrites `turnNumRecord` with a strictly-higher supported turn without finalizing (`ForceMove.sol` L88–119); this is the primitive T6.3 uses. Detail: A.6.

**(d) ForceMove app registry (see A.5 and A.4).**
The set of `appDefinition` addresses T0.3 accepts for a funded channel is defined in `NitroAdjudicator.sol` L193–207, which calls `IForceMoveApp.stateIsSupported` ([A.1.4](./A.1-reuse-inventory.md)). C registers its T4.1 quote/settle app here; the skeleton registers only the trivial stand-in (A.8.4 §2). The registry is the extension point by which C's clearing rides A's rail without A knowing the game. A validates only that the channel's `appDefinition` is registered and that its outcome is exit-format-valid.

**(e) Voucher format (see A.6; consumed by B·T2.1 and C).**
`Voucher{ChannelId, Amount, Signature}` carries `Hash() = keccak256(abi(Destination, Uint256))` and `Sign/RecoverSigner` ([A.1.3](./A.1-reuse-inventory.md) `vouchers.go` L23–52). This is the incremental in-channel micropayment primitive: B meters watcher service against it (T2.1), and C meters settlement against it. It is redeemed by concluding the channel at the largest signed amount.

**(f) Pool commitment-insert and nullifier-spend event ABI, plus circuit artifacts (see A.2 and A.3; consumed by B·T1.2, T6.1, T6.6).**
- Events `Shield`, `Transact`, `Nullified` (and `Action`/`Unshield`) at `RailgunLogic.sol` L57–77 ([A.1.1](./A.1-reuse-inventory.md)) are the commitment-insert and nullifier-spend feed that B indexes (T1.2) and that the note-scanner rebuilds the tree from (T6.1). Tree params: `TREE_DEPTH=16`, `rootHistory` (`Commitments.sol` L28–55).
...
- **B's proof-carrying feed** — `getStorageAt` returning `{value, proof}` (B's T2.0 surface). A's settlement client (T6.2) and watchtower (T6.3) read the adjudicator's `statusOf[channelId]` and holdings and the pool roots through it, rather than trusting a bare RPC, so that a light client verifies chain state under the anonymity-set constraints.
- **B's freshness signal — the T6.3 gate.** The per-contract head-cursor freshness signal B publishes is a hard gate on T6.3: the watchtower MUST NOT decide it holds a higher-turn state, and it MUST NOT sleep through a `finalizesAt` window, on stale data. The registry records T6.3 as "gated on T2 feed freshness." If the feed is stale beyond the safety margin relative to `finalizesAt`, T6.3 escalates or alerts rather than assuming safety. Contract detail is owned by B; A's obligation is to block on it. See A.6.
- **D hosts A** — the settlement client (T6.2), watchtower loop (T6.3), and note-scanner bridge run inside D's wallet/app shell (T6.0/T6.5). A publishes the client edges; D provides transport and lifecycle.

C consumes A (settlement), but it does so through A.8.5.1(a)/(c)/(d)/(e); from A's side that is an exposed surface, not a dependency.
...
Pass means a note's value made a full round-trip through a Nitro channel and re-emerged as fresh, scannable notes, with `sumIn == sumOut` across the boundary and no orphaned holdings. This proves *notes-in-Nitro-notes-out* before any item is deepened.
