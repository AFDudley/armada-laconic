...
- **Aave-v4 yield.** This adapter turns a shielded USDC note into a shielded aUSDC ("aave") note. It is the mechanism for USDC yield, which — since Armada is privacy-for-USDC — is the priority capability (work package C open decision; see [ADR-0013](./09-architecture-decisions.md#adr-0013) and the C open-decisions list in [`00-work-packages.md`](./00-work-packages.md)). We build it (or adapt the Railgun Cookbook recipe pattern for an Aave recipe), C-scope.
- **Swaps.** A token-for-token exchange exposed as an application on the venue. Like the yield recipe, it is build (or adapt the Railgun Cookbook recipe pattern); the swap itself clears over Nitro like any other T4 trade. Routing is T5.1.
