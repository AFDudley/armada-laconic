...
The watchtower is **the same T6.1 note-scanner loop watching a different event**: it subscribes to T0.2's adjudicator `Challenge` events (delivered over the T2 feed) and, on a challenge that would finalize a **stale** state, submits the user's **higher-turn co-signed state** through T0.3's `respond` / `checkpoint` path within the challenge window. Non-custody means a dead counterparty freezes trading, never funds — but only if *someone* answers the challenge for an offline user, which is exactly this loop's job.

**A phone must be able to run it, and does.** It needs only (a) the feed subscription the wallet already maintains and (b) the ability to sign + submit a single `respond` transaction — the same call T6.2 already drives, relayed gaslessly via the keeper/broadcaster (T6.5). With no prover and no heavy state, it is a lightweight reactive loop inside the wallet process, phone-deployable by construction. Users wanting stronger liveness can additionally delegate the identical loop to an always-on watcher party (T2), but the default self-hosted loop runs on the device.
...
**Key tasks.** Subscribe to `Challenge` over the T2 feed; on a stale-finalizing challenge, sign + submit the higher-turn state through T0.3 `respond` within the window, relayed gaslessly (T6.5); add the feed-freshness alarm + redundant-party fallback.
