...
`laconic_asset_inventory.md`. Where the audit
...
Armada does not build the indexing, payment-metering, or client
...
1. **Anonymity-preserving sync (watchers).** A shielded pool's privacy dies at
   ...
   clients scan locally and privately. This protects the pool's shared
...
2. **Metered payments + clearing (Nitro).** Armada's model is metered flows with
   ...
   settle to L1 in USDC. No marketplace and no fee vault, just clearing.
...
This reuses what Laconic already built: verifiable Ethereum
...
- Laconic can charge on its own rail, so its revenue does not depend on Armada's fee
  switch; the two fee surfaces merge into one coordinated schedule.
- The collaboration is operation and fee-sharing: Laconic runs the substrate and
  earns from served volume rather than handing over a codebase.
